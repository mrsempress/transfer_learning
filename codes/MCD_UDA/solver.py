...
class Solver(object):
    def __init__(self, use_abs_diff, eval_only, resume_epoch, batch_size=64, source='svhn',
        ...
        self.datasets, self.dataset_test = Load_data.dataset_read(source, target, self.batch_size, scale=self.scale,
                                                        all_use=self.all_use)
        print('load finished!')
        # one generator and two classifiers
    ...
    def test(self, epoch, record_file=None, save_model=False):
        ...
        print(
            '\nTest set: Average loss: {:.4f}, Accuracy C1: {}/{} ({:.0f}%) Accuracy C2: {}/{} ({:.0f}%) Accuracy '
            'Ensemble: {}/{} ({:.0f}%) \n'.format(
