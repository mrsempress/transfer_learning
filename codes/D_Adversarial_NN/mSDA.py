# encoding=utf-8
"""
    Created on 13:36 2019/07/23
    @author: Chenxi Huang
    It implements "Domain Adversarial Training of Neural Networks".
    Generate the mSDA representations (these are literal translations of Chen et al. (2012) Matlab code)
"""
import numpy as np
...
def compute_msda_representation(x_source, x_target, x_test):
    ...
    new_x_source = new_x_train[:, :nb_souce].T
    new_x_target = new_x_train[:, nb_souce:].T
    new_x_test = msda_forward(x_test.T, W).T
    return new_x_source, new_x_target, new_x_test
